...
var
  ...
  Pan: TPanel;
  dbIndex: integer;
  //ParamTrans:TIBTransaction;
  IsTransActive: boolean;
begin
  //dbIndex := fmMain.PageControl1.ActivePage.Tag;
  dbIndex := TPNodeInfos(fmMain.tvMain.Selected.Data)^.dbIndex;
  //ParamTrans := FSQLTrans; //RegisteredDatabases[dbIndex].IBTransaction;
  ...
  begin
    ...
    case QueryType of
      ...
      begin
        ...
        aSqlQuery:= TIBQuery.Create(self);
        aSqlQuery.DataBase:= FIBConnection; //RegisteredDatabases[dbIndex].IBDatabase;
        aSqlQuery.Transaction:= FSQLTrans; //RegisteredDatabases[dbIndex].IBTransaction;
        aSqlQuery.AllowAutoActivateTransaction := true;
      end;
      qtScript: // Script
      begin
        ...
        aSQLScript.DataBase:= FIBConnection; //RegisteredDatabases[dbIndex].IBDatabase; //FIBConnection;
        aSQLScript.Transaction:= FSQLTrans; //RegisteredDatabases[dbIndex].IBTransaction; //FSQLTrans;
        IsTransActive := FSQLTrans.InTransaction;
        if  IsTransActive then
          FSQLTrans.Commit;
        //FSQLTrans.Params.Assign(ParamTrans.Params);
        if IsTransActive then
          FSQLTrans.StartTransaction;
        ...
        begin
          ...
          try
            ...
            begin
              ...
              IsTransActive := FSQLQuery.Transaction.InTransaction;
              if IsTransActive then
                FSQLQuery.Transaction.Commit;
              //FSQLQuery.Transaction.Params.Assign(ParamTrans.Params);
              if IsTransActive then
            ...
            begin
              ...
              if cxAutoCommit.Checked then
                FSQLQuery.Transaction.CommitRetaining;
...
begin
  dbIndex := TPNodeInfos(fmMain.tvMain.Selected.Data)^.dbIndex;
  ParamTrans := RegisteredDatabases[dbIndex].IBTransaction;
