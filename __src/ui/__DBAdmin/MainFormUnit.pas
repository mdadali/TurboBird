...
begin
  if TPNodeInfos(FNodeInfos)^.ViewForm <> nil then
    TPNodeInfos(FNodeInfos)^.ViewForm := nil;
...
begin
  ...
  DBLoginDlg := TDBLoginDlg.Create(self);
  ShutdownDatabaseDlg := TShutdownDatabaseDlg.Create(self);
  ShutdownReqDlg := TShutdownReqDlg.Create(self);
  BackupDlg := TBackupDlg.Create(self);
  RestoreDlg := TRestoreDlg.Create(self);
  AddSecondaryFileDlg := TAddSecondaryFileDlg.Create(self);
  Application.CreateForm(TAddShadowFileDlg, AddShadowFileDlg);
  AddShadowSetDlg := TAddShadowSetDlg.Create(self);
  NewUserDlg := TNewUserDlg.Create(self);
  ChgPasswordDlg := TChgPasswordDlg.Create(self);
  ExecuteSQLScriptDlg := TExecuteSQLScriptDlg.Create(self);
end;
