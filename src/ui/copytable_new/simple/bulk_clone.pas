...
begin
  ...
  IBDSource := RegisteredDatabases[FSourceDBIndex].IBDatabase;
  IBTransSource := RegisteredDatabases[FSourceDBIndex].IBTransaction;
  IBQuerySource := RegisteredDatabases[FSourceDBIndex].IBQuery;

  if not IBDSource.Connected then
    IBDSource.Connected := true;

  IBDSource.GetTableNames(comboxTables.Items);
...
begin
  ...
  try
    // -----------------------------
    //CREATE EXTERNAL TABLE  (all Formats)
    // -----------------------------
    case GetFormat of
      efBinary,
      efFixedText,
            ...
            begin
              ...
              IBQuerySource.SQL.Text :=
                Format('INSERT INTO %s SELECT FIRST %d SKIP %d * FROM %s',
                       [edtExternalTableName.Text, batchSize, fromRow, comboxTables.Text]);
            end;
        end;
...
begin
  ...
  try
    IBDSource.GetFieldNames(SrcTable, Fields);
