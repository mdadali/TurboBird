...
begin
  ...
  begin
    ShowMessage(FServerSession.ErrorStr);
  end;
...
{  IntPort := StrToIntDef(FPort, 0);
  ...
  try
    FServerSession.Connect;
    if FServerSession.Connected then
      if cbProtocol.Text = UpperCase('LOCAL') then
      begin
        ODSMajorMinorStr := ODSVersionToFBVersion(ODSMajor, ODSMinor);

        FServerSession.FBVersionMajor := StrToInt(ODSMajorMinorStr[1]);
        FServerSession.FBVersionMinor := StrToInt(ODSMajorMinorStr[3]);

        edtVersionString.Text := ServerVersion;
        FServerSession.FBVersionString := ServerVersion;
        edtVersionMajor.Text  := IntToStr(FServerSession.FBVersionMajor);
        edtVersionMinor.Text  := IntToStr(FServerSession.FBVersionMinor);
      end;
  ...
  // Embedded-Verbindung separat prüfen
  if cbProtocol.Text = UpperCase('LOCAL') then
  begin
      Msg := 'Embedded connection successful!';
      DlgType := mtInformation;
  ...
  begin
    ...
    begin
      ...
      Msg := Format('%s%sServer version: %s',
        [FServerSession.ErrorStr, sLineBreak, FServerSession.FBVersionString]);

      DlgType := mtInformation;
      //bbReg.Enabled := True;

      edtVersionString.Text := FServerSession.FBVersionString;
      edtVersionMajor.Text  := IntToStr(FServerSession.FBVersionMajor);
      edtVersionMinor.Text  := IntToStr(FServerSession.FBVersionMinor);
    ...
    begin
      // Fehlertext direkt aus Session übernehmen
      Msg := Format('Server connection failed:%s%s',
        [sLineBreak, FServerSession.ErrorStr]);
