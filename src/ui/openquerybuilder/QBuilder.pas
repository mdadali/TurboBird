...
begin
  ...
  begin
    ...
    begin
      FSQL.Assign(TOQBForm(FOQBForm).SynSQL.Lines);
      Result := True;
    end;
...
begin
  ...
  try
    StrList.LoadFromFile(DlgOpen.FileName);
    if (StrList.Count = 0) or (StrList[0] <> QBSignature) then
    begin
      ShowMessage('File ' + DlgOpen.FileName + ' is not QBuilder''s query file.');
      Exit;
    end;
    ...
    begin
      ...
      begin
        WindowState := wsNormal;
        Top := StrToIntDef(GetNextVal(s), Top);
        Left := StrToIntDef(GetNextVal(s), Left);
        Height := StrToIntDef(GetNextVal(s), Height);
        Width := StrToIntDef(GetNextVal(s), Width);
      end;
    end;
    ...
    begin
      s := StrList[4];
      btnTables.Down := Boolean(StrToIntDef(GetNextVal(s), 0));
      VSplitter.Visible := btnTables.Down;
      QBTables.Visible := btnTables.Down;
      QBTables.Width := StrToIntDef(GetNextVal(s), QBTables.Width);
      btnPages.Down := Boolean(StrToIntDef(GetNextVal(s), 0));
      HSplitter.Visible := btnPages.Down;
      Pages.Visible := btnPages.Down;
      Pages.Height := StrToIntDef(GetNextVal(s), Pages.Height);
    end;
    ...
    begin
      s := StrList[6];
      TempDatabaseName := GetNextVal(s);
      ShowSystemTables := Boolean(StrToIntDef(GetNextVal(s), 0));
      if Assigned(QBDialog) and Assigned(QBDialog.OQBEngine) then
      begin
        QBDialog.OQBEngine.DatabaseName := TempDatabaseName;
        QBDialog.OQBEngine.ShowSystemTables := ShowSystemTables;
      end;
    ...
    begin
      ...
      s := StrList[i];
      TableName := GetNextVal(s);
      Y := StrToIntDef(GetNextVal(s), 0);
      X := StrToIntDef(GetNextVal(s), 0);

      NewTable := TOQBTable.Create(Self);
      NewTable.Parent := QBArea;
      try
        NewTable.Activate(TableName, X, Y);
        if Assigned(NewTable.FLbx) then
        begin
          NewTable.FLbx.FLoading := True;
          for ii := 0 to NewTable.FLbx.Items.Count - 1 do
          begin
            ...
            ss := GetNextVal(s);
            if ss <> '' then
              NewTable.FLbx.Checked[ii] := Boolean(StrToIntDef(ss, 0));
          end;
          NewTable.FLbx.FLoading := False;
        end;
      except
    ...
    if j <> -1 then
      ...
      begin
        ...
        ss := GetNextVal(s);
        Table1 := QBArea.FindTable(ss);
        if Table1 = nil then Continue;

        ss := GetNextVal(s);
        FieldN1 := StrToIntDef(ss, -1);

        ss := GetNextVal(s);
        Table2 := QBArea.FindTable(ss);
        if Table2 = nil then Continue;

        ss := GetNextVal(s);
        FieldN2 := StrToIntDef(ss, -1);

        if (FieldN1 >= 0) and (FieldN2 >= 0) then
        begin
          NewLink := QBArea.InsertLink(Table1, Table2, FieldN1, FieldN2);
          ss := GetNextVal(s);
          NewLink.FLinkOpt := StrToIntDef(ss, 0);
          ss := GetNextVal(s);
          NewLink.FLinkType := StrToIntDef(ss, 0);
        end;
      end;
    ...
    if j <> -1 then
      for i := j to StrList.Count - 1 do
      begin
        if StrList[i] = '[End]' then Break;
        s := StrList[i];

        ii := StrToIntDef(GetNextVal(s), -1);
...
begin
  ...
  if SynSQL.Lines.Text='' then
    btnSQLClick(Sender);
  QBDialog.OQBEngine.CloseResultQuery; // OQB 4.0a
  QBDialog.OQBEngine.SetQuerySQL(SynSQL.Lines.Text);
