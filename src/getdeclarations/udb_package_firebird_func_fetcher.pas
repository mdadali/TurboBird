unit udb_package_firebird_func_fetcher;
...
begin
  Result := '';
  if not IsNativeFirebirdFunction(Conn, FuncName, APackageName) then
  begin
    Result := Format('-- Function "%s.%s" is not a native PSQL package function.', [APackageName, FuncName]);
    Exit;
  end;
        ...
        begin
          ...
          begin
            if ((Pos('END', UpperCase(Trim(lines[i - 1]))) = 1) or (Pos(';', Trim(lines[i - 1])) > 0)) and
               ((Pos('FUNCTION ', UpperCase(Trim(lines[i + 1]))) = 1) or
                (Pos('PROCEDURE ', UpperCase(Trim(lines[i + 1]))) = 1)) then
              Continue
            else
              lines.Delete(i);
          end;
...
var
  ...
  P: Integer;
begin
  Header := GetPackageFirebirdFunctionHeader(Conn, FuncName, APackageName);
  Body := GetPackageFirebirdFunctionBody(Conn, FuncName, APackageName);

  if (Header = '') or (Body = '') then
  begin
    Result := Format('-- Function "%s.%s" not found or not valid.', [APackageName, FuncName]);
    Exit;
  end;
