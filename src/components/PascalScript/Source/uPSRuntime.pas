...
begin
  ...
  FStack.Clear;
  FTempVars.Clear;
  for I := NativeInt(FGlobalVars.Count) - 1 downto 0 do
  begin
...
begin
  for i := NativeInt(FExceptionStack.Count) -1 downto 0 do
  begin
    pp := FExceptionStack.Data^[i];
    pp.Free;
  end;
  for i := NativeInt(FResources.Count) -1 downto 0 do
  begin
  ...
  for i := NativeInt(FExportedVars.Count) -1 downto 0 do
    Dispose(PPSExportedVar(FExportedVars.Data^[I]));
  for I := NativeInt(FProcs.Count) - 1downto 0  do
    TPSProcRec(FProcs.Data^[i]).Destroy;
  FProcs.Clear;
  FGlobalVars.Clear;
  FStack.Clear;
  for I := NativeInt(FTypes.Count) - 1downto 0  do
...
begin
  ...
  begin
    ...
    begin
      for l := NativeInt(FStack.count) -1 downto NativeInt(pp.StackSize) do
        FStack.Pop;
    end;
...
var
  ...
  begin
    if NativeInt(Pos + Len) <= Length(s) then begin
  ...
  var
    ...
    begin
      if (not Read(NameLen, 4)) or (NameLen > Length(s) - NativeInt(Pos)) then
      begin
...
begin
  ...
  begin
    o := NativeInt(atype.RealFieldOffsets[i]);
...
begin
  ...
  for i := 0 to VarArrayHighBound(src, 1) - VarArrayLowBound(src, 1) do begin
    ...
    //r := Pointer(IPointer(r) + NativeInt(DestType.RealSize));
...
begin
  ...
  begin
    ...
    ReallocMem(darr, NativeInt(NewLength * elSize) + SizeOf(TDynArrayRecHeader));
    {$IFDEF FPC}
  ...
  begin
    ...
    GetMem(darr, NativeInt(NewLength * elSize) + SizeOf(TDynArrayRecHeader));
...
var
  ...
  begin
    ...
    case IntoType.BaseType of
      ...
      btS8: tbts8(Into^) := NativeInt(b);
      btU16: tbtu16(Into^) := Cardinal(b);
      btS16: tbts16(Into^) := NativeInt(b);
      btU32: tbtu32(Into^) := Cardinal(b);
      btS32: tbts32(Into^) := NativeInt(b);
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btU32: b := tbts32(var1^) >= NativeInt(tbtu32(Var2^));
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btU16: b := tbts32(var1^) <> tbtu16(Var2^);
                  btS16: b := tbts32(var1^) <> tbts16(Var2^);
                  btProcPtr, btU32: b := tbts32(var1^)<> NativeInt(tbtu32(Var2^));
                  btS32: b := tbts32(var1^) <> tbts32(Var2^);
                  btDouble: b := PSGetReal(Var1, var1type) <> tbtdouble(var2^);
                  btSingle: B := psGetReal(Var1, var1Type) <> tbtsingle(var2^);
                  btExtended: B := psGetReal(Var1, var1Type) <> tbtExtended(var2^);
            ...
            btU32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btS8: tbtU32(var1^) := tbtU32(var1^) + cardinal(NativeInt(tbts8(var2^)));
                  btU16: tbtU32(var1^) := tbtU32(var1^) + tbtu16(var2^);
                  btS16: tbtU32(var1^) := tbtU32(var1^) + cardinal(NativeInt(tbts16(var2^)));
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btU32: tbts32(var1^) := tbts32(var1^) + NativeInt(tbtu32(var2^));
            ...
            btU32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btS8: tbtU32(var1^) := tbtU32(var1^) div cardinal(NativeInt(tbts8(var2^)));
                  btU16: tbtU32(var1^) := tbtU32(var1^) div tbtu16(var2^);
                  btS16: tbtU32(var1^) := tbtU32(var1^) div cardinal(NativeInt(tbts16(var2^)));
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btU32: tbts32(var1^) := tbts32(var1^) div NativeInt(tbtu32(var2^));
            ...
            btU32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btS8: tbtU32(var1^) := tbtU32(var1^) mod cardinal(NativeInt(tbts8(var2^)));
                  btU16: tbtU32(var1^) := tbtU32(var1^) mod tbtu16(var2^);
                  btS16: tbtU32(var1^) := tbtU32(var1^) mod cardinal(NativeInt(tbts16(var2^)));
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btU32: tbts32(var1^) := tbts32(var1^) mod NativeInt(tbtu32(var2^));
  ...
  case VarType of
    0:
      begin
        ...
        begin
          Param := Cardinal(NativeInt(-PSAddrStackStart) +
            NativeInt(FCurrStackBase) + NativeInt(Param));
    ...
    1: begin
        ...
        FTempVars.FLength := Cardinal(NativeInt(Param) + NativeInt(at.RealSize) + NativeInt(RTTISize + 3)) and not 3;
    ...
    2:
      begin
        ...
        if Param < PSAddrNegativeStackStart then begin
          ...
          Param := Cardinal(NativeInt(-PSAddrStackStart) + NativeInt(FCurrStackBase) + NativeInt(Param));
          if Param >= Cardinal(FStack.Count) then
          begin
            CMD_Err(erOutOfStackRange);
            Result := False;
...
var
  ...
  temp: PPSResource;
begin
  for i := 0 to NativeInt(FResources.Count) -1 do
  begin
    temp := FResources[i];
...
var
  ...
  temp: PPSResource;
begin
  if StartAt > NativeInt(FResources.Count) -1 then
    StartAt := NativeInt(FResources.Count) -1;
...
(*              Dec(FStack.FCount);
              ...
              {$ENDIF}
              FStack.FLength := NativeInt(IPointer(vtemp) - IPointer(FStack.DataPtr));
          ...
          Cm_R: begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    for p := NativeInt(FStack.count) -1 downto NativeInt(pp.StackSize) do
                      FStack.Pop
                  end;
                  FCurrStackBase := pp.BasePtr;
                  if pp.FinallyOffset <> InvalidVal then
...
//    if ExEx <> erNoError then FStatus := FOldStatus;
  ...
  if FStatus = isLoaded then begin
    for I := NativeInt(FStack.Count) - 1 downto 0 do
      FStack.Pop;
    FStack.Clear;
...
begin
 ...
 Result := x^[NativeInt(Ptr)];
 {$ELSE}
 Result := PPtrArr(PPointer(FSelf)^)^[NativeInt(Ptr)];
 {$ENDIF}
end;
...
begin
  ...
  Result := x^[NativeInt(Ptr)];
  {$ELSE}
  Result := PPtrArr(FSelf)^[NativeInt(Ptr)];
  {$ENDIF}
end;
...
begin
  ...
  ParamCount := NativeInt((@P.Decl[1])^);
  if NativeInt(Stack.Count) < ParamCount +1 then begin
    Result := False;
    exit;
  end;
  Dec(ParamCount);
  ...
  begin
    n := NewTPSVariantIFC(Stack[NativeInt(Stack.Count) - 2], False);
    if (n.Dta = nil) or (n.aType.BaseType <> btClass) then
    begin
      result := false;
      Caller.CMD_Err(erNullPointerException);
    ...
    Params.Add(NewPPSVariantIFC(Stack[NativeInt(Stack.Count) - 1], True));
    for i := Stack.Count -3 downto NativeInt(Stack.Count) - ParamCount -2 do
    begin
    ...
    n := NewTPSVariantIFC(Stack[NativeInt(Stack.Count) - 1], False);
    if (n.Dta = nil) or (n.aType.BaseType <> btClass) then
    begin
      result := false;
      Caller.CMD_Err(erNullPointerException);
    ...
    Params := TPSList.Create;
    Params.Add(NewPPSVariantIFC(Stack[NativeInt(Stack.Count) - ParamCount - 2], False));

    for i := Stack.Count -2 downto NativeInt(Stack.Count) - ParamCount -1 do
    begin
      Params.Add(NewPPSVariantIFC(Stack[I], False));
    end;
    try
...
begin
  ...
  begin
    ...
    FSelf := Tobject(n.dta^);
    Params := TPSList.Create;
    Params.Add(NewPPSVariantIFC(Stack[NativeInt(Stack.Count) - 2], True));
...
begin
  ...
  begin
    ...
    FSelf := Tobject(n.dta^);
    n := NewTPSVariantIFC(Stack[NativeInt(Stack.Count) - 1], True); // Result
...
{$ELSE}
asm
  push ecx            // Salva ECX originale (3° parametro target)
  push edx            // Salva EDX originale (2° parametro target)

  mov ecx, esp        // Prepara _ECX (puntatore al valore originale di EDX)
  add ecx, 4          // Prepara _EDX (puntatore al valore originale di ECX)

  mov edx, esp        // Prepara Stack (passato in EDX per la convention 'register')
  add edx, 12         // Offset dello stack prima dei PUSH

  // EAX contiene già 'Self'
  call MyAllMethodsHandler2

  add esp, 8          // Ripristina lo stack dai 2 PUSH

  // Gestione del valore di ritorno e pulizia stack PascalScript
  mov edx, [esp]
  add esp, eax
  mov [esp], edx
end;
...
{$ELSE}
// ... qui ci sono le altre funzioni 32 bit ...
...
{$ENDIF}
    ...
    if Res <> nil then DestroyHeapVariant(res);
  end;
...
function MyAllMethodsHandler3(Self: PScriptMethodInfo; _EDX, _ECX: Pointer; Stack: PPointer): Integer;
var
  Decl: tbtString;
  I, C, regno: Integer;
  Params: TPSList;
...
begin
  ...
  // Gestione Parametri in Registro (EDX, ECX)
  s := decl;
  grfw(s);
  for i := c - 1 downto 0 do
  begin
  ...
  // Gestione Parametri nello Stack
  s := decl;
  grfw(s);
  for i := c - 1 downto 0 do
  begin
  ...
  // Setup Frame d'Eccezione ed Esecuzione
  ex := TPSExceptionHandler.Create;
  ex.FinallyOffset := InvalidVal;
  ex.ExceptOffset := InvalidVal;
  ex.Finally2Offset := InvalidVal;
...
begin
  if ItemNo < 0 then
    val := Items[NativeInt(ItemNo) + NativeInt(Count)]
  else
    val := Items[ItemNo];
...
begin
  ...
  if FLength mod NativeInt(PointerSize) <> 0 then
    //FLength := FLength + (PointerSize - (FLength mod PointerSize));
    FLength := FLength + (NativeInt(PointerSize) - NativeInt((FLength mod NativeInt(PointerSize))));
