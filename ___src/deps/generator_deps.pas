...
begin
  ...
  try
    ...
    ObjectTypes[0].SQLText :=
      ...
      'WHERE UPPER(COALESCE(RDB$PROCEDURE_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';

    // Triggers
    ObjectTypes[1].Name := 'Triggers';
    ObjectTypes[1].ObjType := 'TRIGGER';
    ObjectTypes[1].SQLText :=
      'SELECT RDB$TRIGGER_NAME AS OBJ_NAME FROM RDB$TRIGGERS ' +
      'WHERE UPPER(COALESCE(RDB$TRIGGER_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';
    ...
    ObjectTypes[2].SQLText :=
      ...
      'AND UPPER(COALESCE(RDB$VIEW_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';
    ...
    ObjectTypes[3].SQLText :=
      ...
      'WHERE UPPER(COALESCE(RDB$COMPUTED_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';
    ...
    ObjectTypes[4].SQLText :=
      ...
      'WHERE UPPER(COALESCE(RDB$PACKAGE_HEADER_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';
    ...
    ObjectTypes[5].SQLText :=
      'SELECT RDB$PACKAGE_NAME AS OBJ_NAME FROM RDB$PACKAGES ' +
      'WHERE UPPER(COALESCE(RDB$PACKAGE_BODY_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';

    // Domains (RDB$FIELDS default source)
    ...
    ObjectTypes[6].SQLText :=
      ...
      'WHERE UPPER(COALESCE(RDB$DEFAULT_SOURCE, '''')) CONTAINING ''NEXT VALUE FOR ' + GenNameUpper + '''';

    // Table Default Values (field defaults)
    ...
    begin
      ...
      try
        ...
        begin
          ...
          ResultLines.Add(Format('%s "%s" uses generator "%s". [%s]',
            [ObjectTypes[i].ObjType, ObjectName, AGeneratorName, ObjectTypes[i].Name]));
          Q.Next;
        end;
      except
        on E: Exception do
